...
build.python.silho <- function(parameters) {
  ...
  # while(f<=parameters$Config$Number.Folds){
    ...
    while(g <= best.part.info.f$num.group) {
      ...
      # nrow(train.dataset.cluster)
      # ncol(train.dataset.cluster)
      ...
      # nrow(test.dataset.cluster)
      # ncol(test.dataset.cluster)
      ...
      # nrow(val.dataset.cluster)
      # ncol(val.dataset.cluster)
...
gather.eval.python.silho <- function(parameters) {
  ...
  str = "ESTE É O HPML-ECC - O encadeamento ocorre apenas no nível de rótulos dentro de cada cluster"
